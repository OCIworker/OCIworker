...
| WebSSH 依赖 Docker                         | 旧方案常见                    | 主程序内置，无 Docker 依赖          |
...
或重跑 install.sh —— 它会自动识别为升级模式，**只换 JAR（终端已并入主程序），不动 application.yml 和数据库**；升级时会停用并清理旧版终端组件（若存在）。
...
| 检测到旧版终端组件（含历史 Docker 容器） | 升级时提示并清理，避免与内置终端冲突               |
| 数据库表结构差异                       | 后端启动时自动 ALTER                               |

## 安全提醒

- WebSSH 已并入主程序，访问主端口即可使用内置终端功能
- 云厂商安全组只放行 Web 端口（默认 8818），不要额外暴露数据库或历史终端端口
